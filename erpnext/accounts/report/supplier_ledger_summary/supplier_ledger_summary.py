	...
	def get_data(self):
		...
		for gle in self.gl_entries:
			...
			if(gle.voucher_type == "Purchase Invoice"):
				invoiceno=frappe.get_doc("Purchase Invoice",gle.voucher_no)
				
				for i in invoiceno.taxes:
					if "TDS" in i.account_head or "TCS" in i.account_head:                    
						tds_val[gle.party] = (tds_val.get(gle.party) or 0) +i .base_tax_amount
				self.party_data[gle.party].before_tcs = self.party_data[gle.party].invoiced_amount - (tds_val.get(gle.party) or 0)            
				self.party_data[gle.party].tds_amount = self.party_data[gle.party].invoiced_amount - self.party_data[gle.party].before_tcs
